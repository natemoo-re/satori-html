...
describe("svg", () => {
  ...
  it("should handle style", async () => {
    const result = await satori(
      html`<div>Hello world</div><style>div { color: red; }</style>`,
      {
        width: 100,
        height: 100,
        fonts,
  ...
  it("should handle tailwind", async () => {
    ...
    <div class="flex flex-col md:flex-row w-full py-12 px-4 md:items-center justify-between p-8">
      ...
      <div class="mt-8 flex md:mt-0">
        <div class="flex rounded-md shadow">
          <a href="#" class="flex items-center justify-center rounded-md border border-transparent bg-indigo-600 px-5 py-3 text-base font-medium text-white">Get started</a>
        </div>
        <div class="ml-3 flex rounded-md shadow">
          <a href="#" class="flex items-center justify-center rounded-md border border-transparent bg-white px-5 py-3 text-base font-medium text-indigo-600">Learn more</a>
        </div>
      </div>
    </div>
  </div>
